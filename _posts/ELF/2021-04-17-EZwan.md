...
On lines 12 and 13, there are two keys that appear to be xored by 0x55 in the for loop on line 16.

I write a simple program in C to see if I can crack the first key. **Remember** - hex goes in “backwards,” so the first byte of _local_1a_ is 0x55.
...
_local_20_ holds a value that is compared by the _for loop_, yet in reality, because or incrementing RAX, it’s pointing to a byte that’s one ahead of the counter?
